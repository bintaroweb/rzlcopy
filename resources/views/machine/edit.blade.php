    ...
    <div class="row justify-content-center">
        ...
        <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Edit Mesin Fotocopy</h4>
                    <p class="text-secondary">{{ $customer_name }}</p>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ url('/machines/'. $machine_id . '') }}">
                        <input type="hidden" name="customer" value="{{ $customer_id }}" />
                        @csrf
                        @method('put')
                        <div class="form-group">
                            <label for="name" class="required">Tipe Mesin</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('cogs') is-invalid @enderror" id="date" name ="date" value="{{ $date }}">
                            @error('cogs')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-select" name="status">
                                @foreach ($status as $value)
                                    <option value="{{ $value->id }}" {{ $value->id == $status_id  ? 'selected' : ''}}>{{ $value->status }}</option>
                                @endforeach
